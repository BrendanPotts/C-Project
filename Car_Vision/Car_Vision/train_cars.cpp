...
const std::string car_directory = argv[1];
...
std::vector<std::vector<rectangle> > car_boxes_train, car_boxes_test;
...
// file.
load_image_dataset(images_train, car_boxes_train, car_directory + "/car_training.xml");
load_image_dataset(images_test, car_boxes_test, car_directory + "/car_testing.xml");
...
upsample_image_dataset<pyramid_down<2> >(images_train, car_boxes_train);
upsample_image_dataset<pyramid_down<2> >(images_test, car_boxes_test);
...
add_image_left_right_flips(images_train, car_boxes_train);
...
// seconds to train.
object_detector<image_scanner_type> detector = trainer.train(images_train, car_boxes_train);

// Now that we have a face detector we can test it.  The first statement tests it
// on the training data.  It will print the precision, recall, and then average precision.
cout << "training results: " << test_object_detection_function(detector, images_train, car_boxes_train) << endl;
...
cout << "testing results:  " << test_object_detection_function(detector, images_test, car_boxes_test) << endl;
